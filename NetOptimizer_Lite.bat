...
chcp 65001 >nul
title NetOptimizer Lite v3.3 - By ALI SAKKAF
...
if exist "%SCRIPT_NAME%.tmp" (
    ...
    if "%VALID_DOWNLOAD%"=="1" (
        ...
        echo   [!] Downloaded file is corrupted or invalid. Going to Menu...
        timeout /t 2 >nul
        goto MENU
    )
) else (
    echo   [!] Download failed. Going to Menu...
    timeout /t 2 >nul
    goto MENU
)
...
echo.
echo ==============================================================================
echo                      ULTIMATE NETWORK ^& UPDATE KILLER (LITE) v3.3
...
echo [INFO] Clearing Active BITS Transfer Queues ^& Resetting SCM Failure Recovery Actions...
...
echo [INFO] Hard Locking Service Registry Keys ^& Applying SCM Locks...
...
echo [INFO] Applying Group Policy Locks for BITS, Windows Update ^& Delivery Optimization...
...
echo [INFO] Restoring Service Startup Types ^& SCM Recovery Actions...
for %%S in (%SVC_UPDATE_CORE% %SVC_TELEMETRY%) do (
...
echo [INFO] Removing Group Policy Locks for BITS, Windows Update ^& Delivery Optimization...
...
if "%IS_CHR%"=="0" (
    ...
    if not errorlevel 1 set "ST_CHR=[  BLOCKED  ]"
...
if "%IS_EDG%"=="0" (
    ...
    if not errorlevel 1 set "ST_EDG=[  BLOCKED  ]"
...
if "%IS_BRV%"=="0" (
    ...
    if not errorlevel 1 set "ST_BRV=[  BLOCKED  ]"
...
if "%IS_FF%"=="0" (
    ...
    if not errorlevel 1 set "ST_FF=[  BLOCKED  ]"
...
:FW_STATUS
echo.
echo    Active NetOptimizer Firewall Rules:
echo    ----------------------------------------------------------------------
...
set "RAM_BEFORE_MB="
for /f "delims=" %%M in ('powershell -NoProfile -Command "$ErrorActionPreference='SilentlyContinue'; $mem = try { (Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory } catch { (Get-WmiObject Win32_OperatingSystem).FreePhysicalMemory }; [math]::Round($mem / 1024)" 2^>nul') do set "RAM_BEFORE_MB=%%M"
...
echo.
set "RAM_AFTER_MB="
for /f "delims=" %%M in ('powershell -NoProfile -Command "$ErrorActionPreference='SilentlyContinue'; $mem = try { (Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory } catch { (Get-WmiObject Win32_OperatingSystem).FreePhysicalMemory }; [math]::Round($mem / 1024)" 2^>nul') do set "RAM_AFTER_MB=%%M"
...
echo.
echo [SUCCESS] OneDrive has been terminated and blocked from auto-start.
echo [i] Files in OneDrive folder are untouched.
echo.
pause
goto MENU
...
echo [DONE] Telemetry level set to 0 (Security).
echo.
...
echo [i] Restart required for all changes to take full effect.
echo.
pause
goto MENU
...
:SU_DISABLE
echo.
set /p su_name="   >> Enter exact registry entry name to disable (or 0 to cancel): "
...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        ...
        echo    [SUCCESS] Disabled from HKLM Run: %su_name%
        echo [%date% %time:~0,8%] [STARTUP] Disabled HKLM: %su_name% >> "%LOG_FILE%"
    )
) else (
    echo    [SUCCESS] Disabled from HKCU Run: %su_name%
